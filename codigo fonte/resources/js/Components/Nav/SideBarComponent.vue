<template>
    <aside :class="[
           ...
           class="fixed top-[70px] left-0 z-50 w-64 w-full-mobile h-screen transition-transform -translate-x-full bg-white border-r border-r-gray-200 border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700 custom-side-shadow"
           aria-label="Sidebar">
        <div class="h-full pb-4 overflow-y-auto bg-white dark:bg-gray-800 p-4">
            <ul>
                <li class="mb-3">
                    <div class="flex justify-between w-full gap-4">
                        <button @click.prevent="toggleMissionModal" class="btn-menu-mission">
                            <div class="btn-menu-mission-text">
                                <h1>{{ $t('Mission') }}</h1>
                            </div>
                            <img :src="`/assets/images/quests.png`" alt="" width="38">
                        </button>
                        <button @click.prevent="$router.push('/landing/spin')" class="btn-menu-rotate">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <RouterLink :to="{ name: 'home' }" active-class="category-active" class="bg-gray-300/20 flex flex-col items-center justify-center text-center p-3 hover:dark:bg-gray-700/20 dark:bg-gray-700 rounded-xl">
                        <img :src="`/assets/images/icons/home.svg`" alt="" width="35">
                        <span class="text-[12px] mt-2">{{ $t('Home') }}</span>
                    </RouterLink>
                </div>
                <div>
                    <RouterLink :to="{ name: 'casinos' }" active-class="category-active" class="bg-gray-300/20 flex flex-col items-center justify-center text-center p-3 hover:dark:bg-gray-700/20 dark:bg-gray-700 rounded-xl">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div v-if="isLoading" v-for="i in 10" class="flex items-center justify-center h-20 max-w-sm rounded-xl animate-pulse bg-gray-100 dark:bg-gray-700">

                </div>
                <div v-else v-for="(category, index) in casinoCategories" :key="index">
                    <RouterLink :to="{ name: 'casinosAll', params: { provider: 'category', game: category.id }}" active-class="category-active" class="h-20 bg-gray-300/20 flex flex-col items-center justify-center text-center p-3 hover:dark:bg-gray-700/20 dark:bg-gray-700 rounded-xl">
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                <div v-if="setting && setting.turn_on_football">
                    <RouterLink :to="{ name: 'sports' }" active-class="category-active" class="bg-gray-300/20 flex flex-col items-center justify-center text-center p-3 hover:dark:bg-gray-700/20 dark:bg-gray-700 rounded-xl">
                        <img :src="`/assets/images/icons/soccer.svg`" alt="" width="35">
                        <span class="text-[12px] mt-2">{{ $t('Sports') }}</span>
                    </RouterLink>
                </div>
                <div>
...
export default {
    ...
    methods: {
        ...
        getCasinoCategories: function() {
            ...
            HttpApi.get('casinos/categories/')
                .then(response => {
                    _this.casinoCategories = response.data.casinoCategories;
        ...
        getSportCategories: function() {
            const _this = this;
            const _toast = useToast();

            HttpApi.get('sportscategories')
                .then(response => {
                    _this.sportsCategories = response.data.sportsCategories;
                })
                .catch(error => {
                    Object.entries(JSON.parse(error.request.responseText)).forEach(([key, value]) => {
                        _toast.error(`${value}`);
