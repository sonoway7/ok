<template>
    <GameLayout :simple="false">
        <div class="relative w-full min-h-[calc(100vh-565px)]">
            <div v-if="!isLoading">
                <div class="playgame">
                    <div class="playgame-body">
                        <iframe :src="`https://assets.mick.vsslots.com/launcher/GameManager.php?siteId=valebetsports&gameMode=REAL&currency=USD&channel=desktop&locale=es&gameId=POPEYE6M4&userId=test_user1&token=12Z3GCF8YH-J1JXFAG8-4YP`" class="game-full"></iframe>
                    </div>
                    <div class="action-buttons" style="position: absolute; top: 10px; left: 10px;">
                        <button @click="$router.push('/casinos')" class="w-button btn-small">
                            <i class="fa-regular fa-arrow-left mr-3"></i> {{ $t('Close') }}
                        </button>
                    </div>
                </div>
            </div>
