...
export default {
    props: [],
    components: {GameLayout, AuthLayout},
    data() {
        return {
            isLoading: false,
    ...
    methods: {
        getFiversSingle: async function() {
            const _this = this;
            const route = useRoute();
            _this.isLoading = true;

            const slug = route.params.slug;
            await HttpApi.get('games/fivers/' + slug)
                .then(async response =>  {
                ...
                .catch(error => {
                    if(!_this.isAuthenticated) {
                        _this.router.push({ name: 'login', params: { action: 'openlogin' } });
                    }
